...
static void
g_mime_extra_filter_reply_class_init(GMimeExtraFilterReplyClass *klass,
                                     void *class_data);
static void g_mime_extra_filter_reply_init(GMimeExtraFilterReply *filter,
                                           GMimeExtraFilterReplyClass *klass);
static void g_mime_extra_filter_reply_finalize(GObject *object);
...
GType g_mime_extra_filter_reply_get_type(void) {
  ...
  if (!type) {
    static const GTypeInfo info = {
        .class_size = sizeof(GMimeExtraFilterReplyClass),
        .base_init = NULL,
        .base_finalize = NULL,
        .class_init = (GClassInitFunc)g_mime_extra_filter_reply_class_init,
        .class_finalize = NULL,
        .class_data = NULL,
        .instance_size = sizeof(GMimeExtraFilterReply),
        .n_preallocs = 0,
        .instance_init = (GInstanceInitFunc)g_mime_extra_filter_reply_init,
        .value_table = NULL,
    };
    type = g_type_register_static(GMIME_TYPE_FILTER, "GMimeExtraFilterReply",
                                  &info, 0);
  }
  return type;
}
...
g_mime_extra_filter_reply_class_init(GMimeExtraFilterReplyClass *klass,
  ...
  object_class->finalize = g_mime_extra_filter_reply_finalize;
...
static GMimeFilter *filter_copy(GMimeFilter *filter) {
  GMimeExtraFilterReply *reply = (GMimeExtraFilterReply *)filter;

  return g_mime_extra_filter_reply_new(reply->encode);
}

static void filter_filter(GMimeFilter *filter, char *inbuf, size_t inlen,
                          size_t prespace, char **outbuf, size_t *outlen,
                          size_t *outprespace) {
  GMimeExtraFilterReply *reply = (GMimeExtraFilterReply *)filter;
...
static void filter_complete(GMimeFilter *filter, char *inbuf, size_t inlen,
                            size_t prespace, char **outbuf, size_t *outlen,
                            size_t *outprespace) {
  if (inbuf && inlen)
...
GMimeFilter *g_mime_extra_filter_reply_new(gboolean encode) {
  GMimeExtraFilterReply *new_reply;

  new_reply = (GMimeExtraFilterReply *)g_object_new(
      GMIME_EXTRA_TYPE_FILTER_REPLY, NULL);
